/*
 * 批量归集
 * 在链上交互、撸毛之后, 需要将多个钱包的资产进行归集管理; 可以用HD钱包或者
 * 保存多份密钥的方式操作多个钱包, 然后用ethers.js脚本完成归集;
 *
 */
...
// 7. 将WETH合约连接到新的钱包, 然后调用transfer()方法归集每个钱包的WETH
for (let i = 0; i < numWallet; i++) {
	// TOOD: connect
    // 将钱包连接到provider
    let walletiWithProvider = wallets[i].connect(provider)
...
// 8. 读取一个地址在归集后的ETH和WETH余额, 看到ETH和WETH余额减少, 归集成功
